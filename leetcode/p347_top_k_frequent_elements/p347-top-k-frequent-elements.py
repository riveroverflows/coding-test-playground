...
- 1단계: nums를 순회하며 각 숫자의 등장 횟수를 dict로 센다. O(n).
- 2단계: 빈도수를 인덱스로 하는 버킷 배열을 만든다.
  빈도수는 최대 len(nums)이므로 배열 크기가 고정됨.
  각 숫자를 자신의 빈도수에 해당하는 인덱스에 넣는다.
- 3단계: 버킷 배열을 뒤에서부터(높은 빈도수부터) 순회하며 k개를 수집한다.
- 정렬(O(n log n)) 대신 버킷의 인덱스 자체가 정렬 역할을 하므로 O(n).

TC:
- 1단계 빈도수 세기: nums를 한 번 순회. O(n).
  - dict 삽입/조회: 해시 기반 평균 O(1).
- 2단계 버킷 배열 생성: 고유 원소 수만큼 순회. 최악 O(n).
  - 리스트 append: O(1).
- 3단계 상위 k개 수집: 버킷 배열을 뒤에서부터 순회. 최악 O(n).
- 종합: O(n) + O(n) + O(n) = O(n).

SC:
- counts(dict): 최악 n개의 고유 원소. O(n).
- bucket(list of lists): 크기 n+1. 내부 원소 합계는 고유 원소 수. O(n).
- answer(list): 최대 k개. O(k). k ≤ n이므로 O(n).
- 종합: O(n).
"""
