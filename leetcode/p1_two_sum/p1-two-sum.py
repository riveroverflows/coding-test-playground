...
풀이:
- 배열을 한 번 순회하면서, 각 숫자에 대해 "나와 더해서 target이 되는 짝(complement)"이
  이미 등장했는지를 dict로 확인한다.
- dict에 {숫자: 인덱스}를 저장해두면, complement 조회가 O(1)이므로 전체 O(n).
- complement를 먼저 확인하고, 그 다음에 현재 숫자를 dict에 넣기 때문에
  같은 값이 두 번 나오는 경우(예: [3,3], target=6)에도 정상 동작한다.
...
nums, target = json.loads(input()), int(input())
